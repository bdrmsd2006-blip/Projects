...
// simple cube/box implementation
...
public:
    ...
    virtual bool hit(const Ray& r, double t_min, double t_max, HitRecord& rec) const override {
        ...
        for (int a = 0; a < 3; ++a) {
            double invD = 1.0 / r.dir[a];
            double tNear = (box_min[a] - r.orig[a]) * invD;
            double tFar  = (box_max[a] - r.orig[a]) * invD;

            Vec3 n_near(0, 0, 0);
            n_near[a] = invD < 0 ? 1.0 : -1.0;
...
int main() {
    ...
    // materials setup
    auto mat_ground = std::make_shared<Lambertian>(Color(0.2, 0.8, 0.2));
    auto mat_cube   = std::make_shared<Metal>(Color(0.9, 0.7, 0.2), 0.05);
    auto mat_sphere = std::make_shared<Dielectric>(1.5);
    auto mat_center = std::make_shared<Lambertian>(Color(0.8, 0.2, 0.2));

    // scene geometry
    HittableList world;
    world.add(std::make_shared<Sphere>(Point3(0, -1000.5, -1), 1000, mat_ground));
    world.add(std::make_shared<Box>(Point3(-1.2, 0.0, -1.5), Point3(-0.2, 1.0, -0.5), mat_cube));
    world.add(std::make_shared<Sphere>(Point3(0.6, 0.5, -1.0), 0.5, mat_sphere));
    world.add(std::make_shared<Sphere>(Point3(0.0, 0.2, -0.4), 0.2, mat_center));

    // camera
    Camera cam(Point3(0, 2, 2.5), Point3(0, 0.3, -1), Vec3(0, 1, 0), 50, aspect_ratio);

    // threads render setup
    std::vector<Color> buffer(image_width * image_height);
    std::atomic<int> scanlines_left(image_height);
    const unsigned int threads_count = std::thread::hardware_concurrency();
    std::vector<std::thread> threads;

    auto render_chunk = [&]() {
        while (true) {
            int j = --scanlines_left;
            if (j < 0) break;
    ...
    for (unsigned int t = 0; t < threads_count; ++t) {
        threads.emplace_back(render_chunk);
    }
    ...
    for (int idx = 0; idx < image_width * image_height; ++idx) {
        ...
        auto r = std::sqrt(scale * c.x());
        auto g = std::sqrt(scale * c.y());
        auto b = std::sqrt(scale * c.z());
